...
import type { DodoTermsConditionPageData as TermsConditionSecData } from "@/data/index";
import { FadeIn } from "@/app/components/ui/animations";
import { FaPaw } from "react-icons/fa";


const termsConditionSecData: TermsConditionSecData = (petData.termsConditionSec || {
...
}) as TermsConditionSecData;

export default function TermsConSec() {
    if (!termsConditionSecData) return null;
        ...
        <section className="relative max-w-[1320px] mt-4 sm:mt-6 md:mt-8 lg:mt-10 mx-auto w-full px-4 sm:px-6 lg:px-8">
            ...
            <div className="flex flex-col items-center text-center mb-6">
                ...
                <FadeIn direction="up" delay={0.05}>
                    <h1 className="text-3xl sm:text-4xl lg:text-5xl font-extrabold text-[#3E1408] tracking-tight leading-tight mb-2">
                        {termsConditionSecData.titlePrefix}{" "}
                        <span className="text-[#3E1408]">
                            {termsConditionSecData.titleHighlight}
                        </span>
                    </h1>
                </FadeIn>
            ...
            <FadeIn direction="up" delay={0.15}>
                <div className="w-full text-left font-sans space-y-5">
                    {termsConditionSecData.lastUpdated && (
                        <p className="text-sm sm:text-sm md:text-lg text-[#3E342F] font-normal">
                            <strong className="font-bold text-[#2A211C]">Last Updated:</strong> {termsConditionSecData.lastUpdated}
                        </p>
                    )}
                    {termsConditionSecData.intro && (
                        <p className="text-sm sm:text-sm md:text-lg text-[#4A403A] leading-relaxed font-normal">
                            {termsConditionSecData.intro}
                        </p>
                    )}
                    <div className="space-y-5 pt-2">
                        {termsConditionSecData.sections?.map((item) => (
                    ...
                    {(termsConditionSecData.footerNote || termsConditionSecData.footerSubnote) && (
                        <div className="mt-8 flex flex-col items-center justify-center text-center space-y-1.5">
                            {termsConditionSecData.footerNote && (
                                <div className="inline-flex items-center justify-center gap-2 text-sm sm:text-sm md:text-lg text-[#4A403A] font-normal">
                                    ...
                                    <span>{termsConditionSecData.footerNote}</span>
                                </div>
                            )}
                            {termsConditionSecData.footerSubnote && (
